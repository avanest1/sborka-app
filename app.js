...
function send(payload){if(!tg||typeof tg.sendData!=='function'){notice('Откройте приложение кнопкой в личном чате с ботом.');return}if(adminView){const target=el('admin-employee').value;if(!staff.includes(target)){notice('Сначала выберите сотрудника.');el('admin-employee').focus();return}payload.employee=target}const raw=JSON.stringify(payload);if(new TextEncoder().encode(raw).length>4096){notice('Комментарий слишком длинный для отправки.');return}tg.sendData(raw)}
